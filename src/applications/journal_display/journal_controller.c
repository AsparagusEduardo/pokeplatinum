...
int JournalController_Init(ApplicationManager *appMan, int *state)
{
    ...
    saveData = OverlayManager_Args(appMan);
    journalManager = OverlayManager_NewData(appMan, sizeof(JournalManager), HEAP_ID_JOURNAL);
...
{
    JournalManager *journalManager = OverlayManager_Data(appMan);
