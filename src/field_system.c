...
static BOOL InitFieldSystemContinue(ApplicationManager *appMan, int *state)
{
    ApplicationArgs *argv = OverlayManager_Args(appMan);
...
static BOOL ExecuteFieldProcesses(ApplicationManager *appMan, int *state)
{
    if (HandleInputsEventsAndProcesses(OverlayManager_Data(appMan))) {
        return TRUE;
    } else {
        return FALSE;
    }
...
static FieldSystem *InitFieldSystem(ApplicationManager *appMan)
{
    ...
    FieldSystem *fieldSystem = OverlayManager_NewData(appMan, sizeof(FieldSystem), HEAP_ID_FIELDMAP);
    ...
    fieldSystem->saveData = ((ApplicationArgs *)OverlayManager_Args(appMan))->saveData;
...
static void TeardownFieldSystem(ApplicationManager *appMan)
{
    FieldSystem *fieldSystem = OverlayManager_Data(appMan);
