        ...
        if (windowTemplates[i].screen == screen) {
            ...
            if (windowTemplates[i].setup(appData, &appData->windows[i], color) == TRUE) {
                if (windowTemplates[i].entryID) {
                    ...
                    {
                        u32 yOffset = (windowTemplates[i].leftMarginSize == CENTER_TEXT) ? Font_CalcCenterAlignment(windowTemplates[i].font, string, 0, windowTemplates[i].width * 8) : windowTemplates[i].leftMarginSize;
