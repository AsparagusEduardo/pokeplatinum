...
static BOOL sub_02041D98(FieldSystem *fieldSystem, int param1, int param2)
{
    ...
    if (param1 == 0) {
        if (!sub_02029D10(clips, param2)) {
            return FALSE;
        }
    } else {
        if (!sub_02029D2C(clips, param2)) {
            return FALSE;
        }
    }
...
{
    u16 v0 = ScriptContext_GetVar(ctx);
    ImageClips *clips = SaveData_GetImageClips(ctx->fieldSystem->saveData);
    ImageClipsPhoto *v2 = ImageClips_GetImageClipsPhoto(clips, 0);
...
static BOOL ScrCmd_AddAccessory(ScriptContext *ctx)
{
    ...
    ImageClips *clips = SaveData_GetImageClips(ctx->fieldSystem->saveData);
    UnkStruct_02029D04 *v1 = sub_02029D04(clips);
